...
struct MealsManagerSheet: View {
    ...
    private func addItem() {
        ...
        let cat = store.suggestedCategory(for: name) ?? guessedCategory(for: name) ?? "Onbekend"
        items.append(MealItem(name: name, category: canonicalCategory(cat)))
        newItemName = ""
    }
    ...
    private func addEditItem() {
        ...
        guard !name.isEmpty else { return }
        let cat = store.suggestedCategory(for: name) ?? guessedCategory(for: name) ?? "Onbekend"
        editItems.append(MealItem(name: name, category: canonicalCategory(cat)))
        editNewItemName = ""
    }
